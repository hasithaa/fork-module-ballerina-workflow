...
/**
 * Registers the lifecycle task that packs the workflow descriptor into the generated JAR after
 * code generation completes.
 *
 * @since 0.9.0
 */
public class DescriptorLifecycleListener extends CompilerLifecycleListener {

    private final WorkflowDescriptorStore store;

    public DescriptorLifecycleListener(WorkflowDescriptorStore store) {
        this.store = store;
    }
