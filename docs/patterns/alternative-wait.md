...
The workflow itself does not "publish" channel names to users automatically. Channel names and callback endpoint details are part of your integration contract (for example, embedded in approval links, notification payloads, or documented API contracts).
...
How approvers discover this endpoint is integration-specific: it is typically shared by deployment configuration, notification content generated by `notifyApprovers`, or an API gateway contract.
...
- If data was already sent before the restart, it is delivered immediately on replay from durable history.
- If multiple responses are sent while the workflow is recovering, arrivals are recorded deterministically by the engine. With the shared-channel first-wins model, the first recorded completion unblocks the workflow and later responses are ignored.
...
| **Use case** | Any-approver, first-responder | Dual authorization (Four-Eyes Principle), collect all inputs |
