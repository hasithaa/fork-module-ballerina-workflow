...
- `createEventsRecord(RecordType, SignalAwaitWrapper, Scheduler)` — iterates over record fields, creates a `TemporalFutureValue` for each, returns a populated `BMap` record
